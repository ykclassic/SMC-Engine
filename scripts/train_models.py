...
def build_labels(frame, config: dict) -> np.ndarray:
    ...
    event_direction = {
        "BULLISH_SWEEP": 1, "BULLISH_BOS": 1, "BULLISH_CHOCH": 1, "BULLISH_FVG": 1, "BULLISH_OB": 1,
        "BEARISH_SWEEP": -1, "BEARISH_BOS": -1, "BEARISH_CHOCH": -1, "BEARISH_FVG": -1, "BEARISH_OB": -1,
    }
...
def train(config: dict) -> None:
    ...
    for features, labels, indices, split1, split2 in datasets:
        for destination_x, destination_y, selected in ((train_x, train_y, indices[:split1]), (val_x, val_y, indices[split1:split2]), (test_x, test_y, indices[split2:])):
            ...
            destination_x.append(x); destination_y.append(y)
    ...
    for epoch in range(20):
        ...
        loss = criterion(model(X_train), y_train); loss.backward()
    ...
    for threshold in thresholds:
        ...
        tp = ((pred == 1) & (val_truth == 1)).sum(); fp = ((pred == 1) & (val_truth == 0)).sum(); fn = ((pred == 0) & (val_truth == 1)).sum()
