...
def validate() -> int:
    ...
    minimum_precision = float(model_cfg.get("minimum_test_precision", 0.0))
    if float(test.get("roc_auc", 0.0)) < minimum_auc:
        print(f"Validation failed: test ROC-AUC {test.get('roc_auc')} < {minimum_auc}")
        return 1
    if float(test.get("precision", 0.0)) < minimum_precision:
        print(f"Validation failed: test precision {test.get('precision')} < {minimum_precision}")
        return 1
