...
def validate() -> int:
    """Validate model files, feature schema, hashes, and test gates."""
    ...
    if metadata.get("feature_version") != FEATURE_VERSION:
        print(
            "Validation failed: feature version mismatch: "
            f"metadata={metadata.get('feature_version')!r}, "
    ...
    if sequence_length != int(model_cfg["sequence_length"]):
        print(
            "Validation failed: sequence length mismatch: "
            f"metadata={sequence_length}, "
            f"config={model_cfg['sequence_length']}"
        )
        return 1
    ...
    try:
        ...
        minimum_precision = float(
            model_cfg.get("minimum_test_precision", 0.45)
        )
    ...
    if test_auc < minimum_auc or test_precision < minimum_precision:
        print(
            ...
            f"required ROC-AUC>={minimum_auc:.4f} "
            f"precision>={minimum_precision:.4f}"
        )
        return 1
