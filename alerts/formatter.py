...
class SignalFormatter:
    ...
    def format_discord_embed(signal: TradingSignal | dict, config: dict) -> dict:
        ...
        color = (
            config["discord"]["color_long"]
            if data["side"] == "LONG"
            else config["discord"]["color_short"]
        )
        ...
        if ai_enabled:
            fields.append(
                {
                    "name": "AI Confidence",
                    "value": f"{data['ai_confidence'] * 100:.1f}%",
                    "inline": True,
                }
            )
        else:
            fields.append(
                {
                    "name": "Signal Mode",
                    "value": "DETERMINISTIC SMC | AI DISABLED",
                    "inline": True,
                }
            )
        ...
        fields.extend(
            [
                {
                    "name": "Confluence",
                    "value": f"{data['confluence_score'] * 100:.1f}%",
                    "inline": True,
                },
                {
                    "name": "MTF Bias",
                    "value": f"Daily: {data['daily_bias']} | H4: {data['h4_bias']}",
                    "inline": False,
                },
                {
                    "name": "Trigger",
                    "value": f"{data['m15_trigger']} | {data['timeframe']}",
                    "inline": False,
                },
            ]
        )
