...
def main() -> int:
    ...
    for symbol in symbols:
        ...
        summary["scanned"] += 1
        try:
            # Live scans must use the paginated history path. A single CCXT
            # fetch_ohlcv request can be silently capped by the exchange
            # below the configured engine context, producing incomplete
            # higher-timeframe data and a false production failure.
            daily = exchange.fetch_ohlcv_history(
                symbol,
                timeframes["daily"],
                limits["daily"],
            )
            h4 = exchange.fetch_ohlcv_history(
                symbol,
                timeframes["h4"],
                limits["h4"],
            )
            h1 = exchange.fetch_ohlcv_history(
                symbol,
                timeframes["h1"],
                limits["h1"],
            )
            m15 = exchange.fetch_ohlcv_history(
                symbol,
                timeframes["m15"],
                limits["m15"],
            )
            ...
            if journal.has_recent_signal(symbol, signal.side, cooldown):
                ...
                logger.info(
                    "Suppressed duplicate %s signal for %s",
                    signal.side,
                    symbol,
                )
                continue
            ...
            if delivered:
                ...
                logger.error(
                    "Signal %s was validated but Discord delivery failed: %s",
                    signal.signal_id,
                    status,
                )
